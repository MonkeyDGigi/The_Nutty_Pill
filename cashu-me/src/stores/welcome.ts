...
export const useWelcomeStore = defineStore("welcome", {
  ...
  actions: {
    /**
     * Checks if wallet exists and updates showWelcome accordingly.
     * Should be called when the store is initialized.
     */
    checkWalletAndUpdateWelcome() {
      try {
        if (typeof window !== "undefined" && window.localStorage) {
          const mnemonic = localStorage.getItem("cashu.mnemonic");
          const hasWallet = !!(mnemonic && mnemonic.length > 0);
          
          // If no wallet exists, always show welcome
          if (!hasWallet) {
            this.showWelcome = true;
          }
        }
      } catch (e) {
        console.warn("[WelcomeStore] Error checking wallet:", e);
