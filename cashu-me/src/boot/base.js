...
window.windowMixin = {
  ...
  created: function () {
    ...
    this.g.allowedThemes = window.allowedThemes ?? [
      "retro", "ios", "windows95", "matrix", "neon", "kawaii"
    ];
    ...
    // Set theme - check if already set (from inline script) or set it now
    const currentTheme = document.body.getAttribute("data-theme");
    if (this.$q.localStorage.getItem("cashu.theme")) {
      const theme = this.$q.localStorage.getItem("cashu.theme");
      if (currentTheme !== theme) {
        document.body.setAttribute("data-theme", theme);
      }
    } else if (!currentTheme) {
      // Only set if not already set (might be set by inline script)
      this.changeColor("neon");
    }
